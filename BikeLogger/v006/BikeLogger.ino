...
#include "bme280_module.h"   // BME280 Atmospheric Sensor (🌡️ TEMP/HUM/PRESS)
#include "ina260_module.h"   // INA260 Power Monitor (⚡ VOLTAGE/CURRENT)
#include "ads1115_module.h"  // ADS1115 ADC (📊 ANALOG INPUTS)
#include "rtc_module.h"      // DS3231 RTC (🕐 TIMESTAMPS)
#include "sd_logger.h"       // SD Card Logger (💾 DATA LOGGING)
...
void setup() {
    ...
    // FIXME: Critical Bug #3 - Mutex initialization with proper error handling
    telemetryMutex = xSemaphoreCreateMutex();
    if (telemetryMutex == NULL) {
        Serial.println("❌ FATAL: Mutex konnte nicht erstellt werden!");
        // Enable watchdog to force restart instead of infinite loop
        esp_task_wdt_init(5, true);  // 5 second watchdog
        esp_task_wdt_add(NULL);
        while(1) {
            delay(100);  // Watchdog will trigger reset
        }
    }
...
// FIXME: Critical Bug #1 - REMOVED DUPLICATE FUNCTION DEFINITIONS
// Only actual implementations are kept below
// ═══════════════════════════════════════════════════════════════════
...
bool initGPS() {
    ...
    // FIXME: Critical Bug #6 - GPS initialization no longer blocks setup()
    // GPS will acquire fix in background via taskGPSLoop
    Serial.println("⏳ GPS initialization started (non-blocking)...");
    Serial.println("   GPS will acquire fix in background");
    
    return true;  // GPS is running, fix will come later
}
...
bool initRTC() {
    ...
    // Versuche RTC mit GPS zu synchronisieren (wenn GPS verfügbar)
    if (system_status.gps_online && gps.hasFix()) {
        rtc.syncWithGPS(gps.getData());
    }
...
bool initSDCard() {
    ...
    if (sdLogger.startLogging()) {
        telemetry.logging_active = true;
        Serial.printf("✓ Data Logging aktiv: %s\n", sdLogger.getCurrentFilename().c_str());
    }
...
void taskIMULoop(void* parameter) {
    ...
    while (true) {
        ...
        if (system_status.imu_online) {
            ...
            if (imu.update()) {
                // FIXME: Critical Bug #2 - Use consistent mutex timeout
                if (xSemaphoreTake(telemetryMutex, pdMS_TO_TICKS(50))) {
...
void taskGPSLoop(void* parameter) {
    ...
    uint32_t gps_startup_time = millis();
    bool startup_fix_logged = false;
        ...
        if (system_status.gps_online) {
            ...
            if (gps.update()) {
                ...
                if (xSemaphoreTake(telemetryMutex, pdMS_TO_TICKS(50))) {
                    ...
                    // Log first fix acquisition
                    if (!startup_fix_logged && telemetry.gps.fix_valid) {
                        startup_fix_logged = true;
                        uint32_t fix_time = millis() - gps_startup_time;
                        Serial.printf("✓ GPS Fix acquired after %lu ms\n", fix_time);
                    }
...
void taskSensorsLoop(void* parameter) {
    ...
    while (true) {
        ...
        // BME280 (1Hz - nur jeden 10. Zyklus)
        if (cycle_counter % 10 == 0 && system_status.bme280_online) {
            if (bme280.update()) {
                // FIXME: Critical Bug #2 - Use consistent mutex timeout
                if (xSemaphoreTake(telemetryMutex, pdMS_TO_TICKS(50))) {
                    telemetry.env = bme280.getData();
                    
                    // GPS Altitude Fusion
                    if (system_status.gps_online) {
                        bme280.setGPSAltitude(telemetry.gps.altitude);
                        telemetry.env.altitude_fused = bme280.getFusedAltitude();
                    }
        ...
        // INA260 (10Hz - jeden Zyklus)
        if (system_status.ina260_online) {
            if (ina260.update()) {
                // FIXME: Critical Bug #2 - Use consistent mutex timeout
                if (xSemaphoreTake(telemetryMutex, pdMS_TO_TICKS(50))) {
                    telemetry.power = ina260.getData();
                    xSemaphoreGive(telemetryMutex);
                }
            }
        }
        
        // ADS1115 (10Hz - jeden Zyklus)
        if (system_status.ads1115_online) {
            if (ads1115.update()) {
                // FIXME: Critical Bug #2 - Use consistent mutex timeout
                if (xSemaphoreTake(telemetryMutex, pdMS_TO_TICKS(50))) {
                    telemetry.analog = ads1115.getData();
                    xSemaphoreGive(telemetryMutex);
                }
            }
        }
        
        // RTC Timestamp Update
        if (system_status.rtc_online) {
            // FIXME: Critical Bug #2 - Use consistent mutex timeout
            if (xSemaphoreTake(telemetryMutex, pdMS_TO_TICKS(50))) {
                telemetry.timestamp = rtc.getTimestamp();
                xSemaphoreGive(telemetryMutex);
            }
        }
...
void taskLoggingLoop(void* parameter) {
    ...
    while (true) {
        ...
        if (system_status.sd_online && sdLogger.isLogging()) {
            // Lock Mutex und kopiere Telemetrie-Daten
            ...
            // Schreibe auf SD-Card (außerhalb Mutex)
            if (!sdLogger.logTelemetry(data_copy)) {
                system_status.sd_errors++;
            }
        }
        
        vTaskDelay(pdMS_TO_TICKS(100));  // 10Hz Logging
    }
}
